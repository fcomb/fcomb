...
object EventService extends LazyLogging {
  ...
  def start()(implicit system: ActorSystem, mat: Materializer, db: Database): ActorRef = {
    if (actorRef eq null) {
  ...
  def props()(implicit mat: Materializer, db: Database) =
...
private[this] class EventServiceActor(implicit mat: Materializer, db: Database)
    extends Actor
    with ActorLogging {
  ...
  def pushRepoEvent(details: EventDetails.PushRepo, createdByUserId: Int): Unit = {
    db.run(EventsRepo.create(details, createdByUserId)).flatMap { event =>
      val body   = Encoder[EventDetails].apply(details).noSpaces
      val entity = HttpEntity(`application/json`, body)
      Source
        .fromPublisher(db.stream(ImageWebhooksRepo
          .findByImageId(details.repoId)))
        .mapAsyncUnordered(2)(webhook =>
