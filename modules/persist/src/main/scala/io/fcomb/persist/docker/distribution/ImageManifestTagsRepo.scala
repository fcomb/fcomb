...
final class ImageManifestTagTable(_tag: Tag)
  ...
  def * =
    (imageId, imageManifestId, tag, updatedAt).mapTo[ImageManifestTag]
}
...
object ImageManifestTagsRepo extends PersistModel[ImageManifestTag, ImageManifestTagTable] {
  ...
  private def findByImageIdAsReponseDBIO(imageId: Int, p: Pagination) = {
    ...
    sortPaginate(q, p)(sortByPF, _._4.desc)
  }
  ...
  def paginateByImageId(imageId: Int, p: Pagination)(
      implicit ec: ExecutionContext): DBIO[PaginationData[RepositoryTagResponse]] =
    for {
