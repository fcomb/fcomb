...
import io.fcomb.models.{OwnerKind, Pagination, PaginationData, User}
...
final class PermissionTable(tag: Tag)
  ...
  def * =
    (id.?, imageId, memberId, memberKind, action, createdAt, updatedAt).mapTo[Permission]
}
...
object PermissionsRepo extends PersistModelWithAutoIntPk[Permission, PermissionTable] {
  ...
  private lazy val findIdByImageAndMemberC = Compiled {
    (imageId: Rep[Int], memberId: Rep[Int], memberKind: Rep[MemberKind]) =>
      findIdByImageAndMemberScope(imageId, memberId, memberKind)
  }

  private lazy val findIdByImageAndUserC = Compiled { (imageId: Rep[Int], userId: Rep[Int]) =>
  ...
  private lazy val findActionByImageAndUserC = Compiled { (imageId: Rep[Int], userId: Rep[Int]) =>
    findIdByImageAndUserDBIO(imageId, userId)
  }

  private lazy val findIdByImageAndGroupC = Compiled { (imageId: Rep[Int], userId: Rep[Int]) =>
  ...
  def findActionByImageAndUserDBIO(
    ...
    findActionByImageAndUserC((imageId, userId)).result.headOption

  private def findActionByUserOrganizationsDBIO(imageId: Rep[Int],
                                                organizationId: Rep[Int],
                                                userId: Rep[Int]) =
    ImagesRepo.organizationAdminsScope
      .filter {
        case ((t, _), ogut) =>
      ...
      .take(1)
      .map(_ => (Action.Manage: Rep[Action]).asColumnOf[Action])
      .subquery

  private def findActionByUserGroupsDBIO(imageId: Rep[Int],
                                         organizationId: Rep[Int],
                                         userId: Rep[Int]) =
    ImagesRepo.groupsScope
      .filter {
        case ((t, _), gut) =>
          t.id === imageId && t.ownerId === organizationId &&
            gut.userId === userId
  ...
  private lazy val findActionByImageAsGroupUserC = Compiled {
  ...
  def findActionByImageAsGroupUserDBIO(imageId: Int, organizationId: Int, userId: Int)(
    ...
    findActionByImageAsGroupUserC((imageId, organizationId, userId)).result.headOption
  ...
  def paginateByImageId(image: Image, p: Pagination)(
      implicit ec: ExecutionContext): DBIO[PaginationData[PermissionResponse]] = {
  ...
  def upsertByImage(image: Image, req: PermissionCreateRequest)(
      implicit ec: ExecutionContext): DBIO[ValidationResult[PermissionResponse]] =
    ...
    userIdByMemberRequestDBIO(member).flatMap {
      case Validated.Valid(user) =>
        ...
        if (memberId == image.owner.id && image.owner.kind === OwnerKind.User)
          ...
          findIdByImageAndUserC((image.getId(), memberId)).result.headOption
            .flatMap {
              case Some(p) =>
                val up = p.copy(
                  action = action,
    ...
    if (image.owner.kind == OwnerKind.Organization) {
      groupIdByMemberRequestDBIO(image.owner.id, member).flatMap {
        case Validated.Valid(group) =>
          ...
          findIdByImageAndGroupC((image.getId(), memberId)).result.headOption
            .flatMap {
              case Some(p) =>
                val up = p.copy(
                  action = action,
  ...
  def findSuggestions(image: Image, q: String)(
      implicit ec: ExecutionContext): DBIO[Seq[PermissionMemberResponse]] =
  ...
  private def destroyByMemberAsValidated(imageId: Int, memberId: Int, memberKind: MemberKind)(
      implicit ec: ExecutionContext) =
    findIdByImageAndMemberC((imageId, memberId, memberKind)).result.headOption.flatMap {
      case Some(p) =>
