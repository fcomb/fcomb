...
object OrganizationsRepo extends PersistModelWithAutoIntPk[Organization, OrganizationTable] {
  ...
  lazy val findByNameC = Compiled { name: Rep[String] =>
  ...
  def findBySlugDBIO(slug: Slug) =
    slug match {
      ...
      case Slug.Name(name) => findByNameC(name)
    }

  def findBySlug(slug: Slug)(implicit ec: ExecutionContext): DBIO[Option[Organization]] =
    findBySlugDBIO(slug).result.headOption
  ...
  def paginateAvailableByUserId(userId: Int, p: Pagination)(
      implicit ec: ExecutionContext): DBIO[PaginationData[OrganizationResponse]] = {
  ...
  private lazy val findRoleByIdAndUserIdC = Compiled { (id: Rep[Int], userId: Rep[Int]) =>
    groupsScope
      .filter {
  ...
  def findWithRoleBySlugDBIO(slug: Slug, userId: Int)(implicit ec: ExecutionContext) =
    for {
      ...
      res <- orgOpt match {
        case Some(org) =>
          findRoleByIdAndUserIdC((org.getId(), userId)).result.headOption.map(r => Some((org, r)))
        case _ => DBIO.successful(None)
      }
    } yield res

  def findWithRoleBySlug(slug: Slug, userId: Int)(implicit ec: ExecutionContext) =
    findWithRoleBySlugDBIO(slug, userId)
  ...
  def findBySlugWithAcl(slug: Slug, userId: Int, role: Role)(
      implicit ec: ExecutionContext): DBIO[Option[Organization]] =
    findBySlugWithAclDBIO(slug, userId, role)

  private lazy val isAdminC = Compiled { (id: Rep[Int], userId: Rep[Int]) =>
    groupsScope.filter {
      case ((t, ogt), ogut) =>
  ...
  def findSuggestions(imageId: Int, orgId: Int, q: String, limit: Long = 16L)(
    ...
    val name = s"$q%".trim
    findSuggestionsC((imageId, orgId, name, limit)).result.map(_.map {
      ...
      case (id, MemberKind.Group, name, _) =>
        PermissionGroupMemberResponse(id = id, name = name)
    })
  }
  ...
  override def validate(org: Organization)(implicit ec: ExecutionContext): ValidationDBIOResult = {
    ...
    val dbioValidations = validateDBIO(
      "name" -> List(unique(uniqueNameC((org.id, org.name))))
    )
