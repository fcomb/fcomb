...
final class BlobFileTable(tag: Tag) extends Table[BlobFile](tag, "dd_blob_files") {
  ...
  def * =
    (uuid, digest, state, retryCount, createdAt, updatedAt, retriedAt).mapTo[BlobFile]
}
...
object BlobFilesRepo {
  ...
  private lazy val isDuplicateByDigestC = Compiled { (uuid: Rep[UUID], digest: Rep[String]) =>
    table.filter(t => t.digest === digest && t.uuid =!= uuid).map(_.state).take(1)
  }

  private def isDuplicateByDigestDBIO(uuid: UUID, digest: String) =
    isDuplicateByDigestC((uuid, digest.toLowerCase)).result.headOption
  ...
  def destroy(uuids: Seq[UUID]) =
    if (uuids.isEmpty) DBIO.successful(0)
    else table.filter(_.uuid.inSetBind(uuids)).delete
  ...
  private lazy val findDeletingC = Compiled {
    table.filter(_.state === (BlobFileState.Deleting: BlobFileState))
  }

  def findDeleting() = findDeletingC.result
}
