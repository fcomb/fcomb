...
final class ImageBlobTable(tag: Tag)
  ...
  def * =
    (id.?, imageId, state, digest, contentType, length, createdAt, uploadedAt).mapTo[ImageBlob]
}
...
object ImageBlobsRepo extends PersistModelWithUuidPk[ImageBlob, ImageBlobTable] {
  ...
  def mount(fromImageId: Int, toImageId: Int, digest: String, userId: Int)(
      implicit ec: ExecutionContext): DBIO[Option[ImageBlob]] =
    runInTransaction(TransactionIsolation.ReadCommitted) {
      findUploadedC((fromImageId, digest)).result.headOption.flatMap {
        case Some(blob) =>
          findUploadedC((toImageId, digest)).result.headOption.flatMap {
  ...
  private lazy val findByImageIdAndUuidC = Compiled { (imageId: Rep[Int], uuid: Rep[UUID]) =>
  ...
  def findByImageIdAndUuid(imageId: Int, uuid: UUID)(implicit ec: ExecutionContext) =
    findByImageIdAndUuidC((imageId, uuid)).result.headOption

  private def uploadedScope(imageId: Rep[Int]) =
    table.filter(t =>
      t.imageId === imageId && t.state === (ImageBlobState.Uploaded: ImageBlobState))

  private lazy val findUploadedC = Compiled { (imageId: Rep[Int], digest: Rep[String]) =>
  ...
  def findUploaded(imageId: Int, digest: String)(implicit ec: ExecutionContext) =
    findUploadedC((imageId, digest)).result.headOption
  ...
  def findAllUploadedIds(imageId: Int, digests: Set[String])(
      implicit ec: ExecutionContext): DBIO[Seq[(UUID, Option[String], Long)]] =
    findAllUploadedDBIO(imageId, digests).map(t => (t.id, t.digest, t.length)).result

  def findAllUploaded(imageId: Int, digests: Set[String])(implicit ec: ExecutionContext) =
    findAllUploadedDBIO(imageId, digests).result

  private lazy val existUploadedByImageIdAndDigestC = Compiled {
  ...
  def completeUploadOrDelete(id: UUID, imageId: Int, length: Long, digest: String)(
      implicit ec: ExecutionContext): DBIO[BlobFileState] =
    runInTransaction(TransactionIsolation.ReadCommitted) {
      existUploadedByImageIdAndDigestC((imageId, digest, id)).result.flatMap { exists =>
        if (exists) {
          for {
            _   <- destroy(id)
  ...
  override def destroy(id: UUID)(implicit ec: ExecutionContext): DBIO[ValidationResultUnit] =
    for {
  ...
  def findByIds(ids: List[UUID]) =
    table.filter(_.id.inSetBind(ids)).result

  def createEmptyTarIfNotExists(imageId: Int)(implicit ec: ExecutionContext): DBIO[Unit] =
    findUploadedC((imageId, emptyTarSha256Digest)).result.headOption
      .flatMap {
  ...
  def tryDestroy(id: UUID)(implicit ec: ExecutionContext): DBIO[Either[String, Unit]] =
    runInTransaction(TransactionIsolation.ReadCommitted) {
      ImageManifestLayersRepo.isBlobLinkedC(id).result.flatMap { isLinked =>
        if (isLinked) DBIO.successful(Left("blob is linked with manifest"))
        else findByIdQuery(id).delete.map(_ => Right(()))
      }
    }

  private lazy val existByDigestC = Compiled { digest: Rep[String] =>
    table.filter(_.digest === digest).exists
  }

  def existByDigest(digest: String)(implicit ec: ExecutionContext): DBIO[Boolean] =
    existByDigestC(digest).result
}
